...
import { nextTick, onBeforeMount, onBeforeUnmount, reactive, watch } from 'vue';
...
interface DialogBasePluginComponent extends ReturnType<useDialogPluginComponent> {
  context: WidgetContext;
  dialogProps: Partial<QDialogOptions>;
}

interface DialogBaseComposable {
  (): DialogBasePluginComponent;
  props: typeof props;
  emits: string[];
}

function compose(): DialogBasePluginComponent {
...
compose.props = props;
compose.emits = useDialogPluginComponent.emits;

export const useDialogBase: DialogBaseComposable = compose;
