...
async function selectNewName(defaultName: string): Promise<string | undefined> {
...
async function loadStoredBackup(): Promise<void> {
  ...
  try {
    busy.value = true;
    messages.value = await sparkStore.loadStoredBackup(props.serviceId, name);
  } finally {
    busy.value = false;
  }
}
...
async function saveStoredBackup(): Promise<void> {
  const name = await selectNewName(`blocks_${props.serviceId}_${cleanDate()}`);
  if (name == null) {
    return;
  }
...
async function uploadStoredBackup(): Promise<void> {
  loadFile(async (values: any) => {
    ...
    try {
      ...
      messages.value = await sparkStore.serviceImport(props.serviceId, data);
    } finally {
      busy.value = false;
    }
  });
...
async function downloadStoredBackup(): Promise<void> {
  ...
  try {
    busy.value = true;
    const data = await sparkStore.downloadStoredBackup(props.serviceId, name);
