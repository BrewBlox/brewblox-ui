...
const durationExp =
  /^(\s*\d*\.?\d+\s*(ms|milliseconds?|s|seconds?|m|mins?|minutes?|h|hours?|d|days?)?\s*)*$/i;
const durationUnitExp =
  /^(ms|milliseconds?|s|seconds?|mins?|minutes?|h|hours?|d|days?)$/i;
...
const unitConversionGroups: UnitGroup[] = [
  {
    ...
    example: 'second',
    test: (s) =>
      /^(ms|milliseconds?|s|seconds?|mins?|minutes?|h|hours?|d|days?)$/.test(s),
    convert: (s) => s,
  },
  {
    name: 'Temp',
  ...
  },
  {
    ...
    test: (s) =>
      /^delta_deg(F|C) ?\/ ?(s|seconds?|mins?|minutes?|h|hours?|d|days?)$/i.test(
        s,
      ),
    convert: (s) => s.replace('delta_', ''),
  },
...
export const canSerialize = (obj: unknown): obj is { toJSON(): any } =>
  obj instanceof Object && 'toJSON' in obj;

export const isBloxField = (obj: unknown): obj is BloxField =>
  obj instanceof Object && '__bloxtype' in obj;
...
export const isDurationUnit = (v: unknown): v is string =>
