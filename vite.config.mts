...
export default defineConfig(({ command, mode }) => {
  ...
  if (isDev) {
    ...
    // Leaf certificates generated by minica expire after two years
    const { validTo } = new X509Certificate(serverHttps.cert as Buffer);
    if (new Date(validTo) < new Date()) {
      console.warn(
        '\nWARNING: the dev certificate expired on ' +
          `${validTo}.\n` +
          'Run "yarn makecert" to generate a new one.\n',
      );
    }
